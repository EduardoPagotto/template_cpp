...
class UniformBuffer {
  public:
    UniformBuffer(const UniformBuffer& o) : max(o.max), last(o.last), increment(o.increment) {
    ...
    UniformBuffer(size_t max, size_t increment = 256) : max(max), last(0), increment(increment) { buffer = malloc(max); }
